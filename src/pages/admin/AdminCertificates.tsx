...
export function AdminCertificates() {
  ...
  // Forma holati (Backend maydonlariga moslangan)
  ...
  const handleAddCertificate = async (e: React.FormEvent) => {
    ...
    formData.append("title", title); // Backend data.title deb qabul qiladi
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-slate-900 border border-slate-800 w-full max-w-md p-6 rounded-2xl shadow-2xl relative">
              ...
              <form onSubmit={handleAddCertificate} className="space-y-4">
                <Input placeholder="Sertifikat nomi" value={title} onChange={(e) => setTitle(e.target.value)} required className="bg-slate-800" />
                <Input placeholder="Bergan tashkilot (Issuer)" value={issuer} onChange={(e) => setIssuer(e.target.value)} required className="bg-slate-800" />
                <Input type="month" value={date} onChange={(e) => setDate(e.target.value)} required className="bg-slate-800 text-white" />
                
                <div className="space-y-2">
                  <label className="text-xs text-slate-400">Sertifikat fayli (IMG yoki PDF)</label>
                  <input 
                    type="file" 
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-900 text-slate-400">
                <tr>
                  ...
                  <th className="p-4">Nomi / Tashkilot</th>
                ...
                {certs.length === 0 ? (
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-slate-900/50 transition-colors group">
                      ...
                      <td className="p-4 text-slate-400">{c.date || c.year}</td>
                      <td className="p-4 text-right">
                        <button onClick={() => handleDelete(c.id)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors">
                          <Trash2 size={18} />
                        </button>
                      </td>
                    </tr>
